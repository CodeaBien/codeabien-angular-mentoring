...
export class CartPageService {
   ...
   */
  private enrichCartWithProducts(
    cartApi: CartApiResponse
    ...
    return this.productApiService.getProducts().pipe(
      map((allProducts: ProductResponse[]) => {
        ...
        const enrichedItems = cartApi.products.map((cartProduct) => {
          ...
          if (!product) {
            throw new Error(
              `Producto con ID ${cartProduct.productId} no encontrado`
            );
          }
  ...
  addProductToCart(
    ...
    if (currentCart) {
      ...
      const existingProductIndex = currentCart.items.findIndex(
